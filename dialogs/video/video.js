...
(function () {
    ...
    function insertSingle() {
        ...
        editor.execCommand('insertvideo', {
            ...
            width: width.value,
            height: height.value,
            poster: editor.getOpt('videoPoster'),
            align: align
    ...
    function insertOnline() {
        editor.execCommand('insertvideo', onlineVideo.getInsertList(), null);
    }
    ...
    UploadFile.prototype = {
        ...
        initUploader: function () {
            ...
            uploader.on('uploadBeforeSend', function (file, data, header) {
                ...
                // 如果是qiniu上传并且不通过php上传就通过ajax来获取token,分片上传设置header
                if ((uploadType == 'qiniu' && isDirect != 'php') || isChunked) {
                    var $file = $('#' + file.id),
                        type = editor.getOpt('uploadSaveType'),
                        path = editor.getOpt('videoPathFormat');
            ...
            uploader.on('uploadAccept', function (obj, ret) {
                ...
                if (ret.offset == videoBlockSize) {
                    ...
                    // 分片偶数时取倒数第3个，否则取倒数第2个
                    var last = (obj.chunks % 2 == 0) ? 3 : 2;
                    // 如果是最后一块,并且最后一块不能与videoBlockSize,设置创建块大小
                    if (obj.chunks - obj.chunk == last) {
                        // 剩余片大小
                        var blockSize = obj.total - obj.end;
                        uploadUrl = chunkUploadUrl + '/mkblk/' + blockSize;
                    }
                    qiniuCtx.push(ret.ctx);
                } else {
                    // 上传每块的第2片url
    ...
    OnlineVideo.prototype = {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    domUtils.on(cancel_span, 'click',function(){
                        ...
                        return _this.removeVideo(key,this);
                    });
                    // end
        ...
        scale: function (video, w, h, type) {
            ...
            if (type == 'justify') {
                if (ow >= oh) {
                    video.width = w;
                    video.height = h * oh / ow;
                    video.style.marginLeft = '-' + parseInt((video.width - w) / 2) + 'px';
                } else {
                    video.width = w * ow / oh;
                    video.height = h;
                    video.style.marginTop = '-' + parseInt((video.height - h) / 2) + 'px';
                }
            } else {
                if (ow >= oh) {
                    video.width = w * ow / oh;
        ...
        // 删除视频的方法
        removeVideo: function (key, obj) {
            var url = editor.getActionUrl(editor.getOpt('removeVideoActionName')),
